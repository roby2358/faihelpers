...
2. For each docmem where the docmem ID does NOT start with "chat_" (i.e., excludes chat-related docmems) and is NOT the root prompt docmem (which is already included, serialized, as the main system prompt):
   ...
   - The system message MUST begin with a pretend invocation line of the form `$ System.docmem_expand_to_context("<startNodeId>")`, labeling the content as expanded docmem output. This header MUST also lead the root prompt system message (using the root prompt docmem ID). `System.docmem_expand_to_context` is not a real agent command — the `System.` prefix marks it as a framework-level call, disincentivizing agents from imitating it — and the agent prompt MUST explain that this line is framework-generated labeling.
   - If the docmem is focused, the message MUST carry a focus marker on a line directly below the pretend invocation, naming the focus node and the docmem root and pointing to `docmem_focus("<docmemId>", "<docmemId>")` to restore the full tree (e.g., `[focus: showing only the subtree of <focusNodeId> within docmem <docmemId>; call docmem_focus("<docmemId>", "<docmemId>") to restore the full tree]`). An unfocused docmem MUST NOT carry a focus marker.
...
4. These docmem context system messages (including the roster message, which leads them) MUST be appended AFTER the chat session messages, at the end of the message list. This keeps the append-only conversation history a stable prefix for prompt caching; docmem writes only invalidate the tail of the message list.
...
6. After the docmem context messages, the message list MUST end with a user-role turn message consisting of the pretend invocation line `$ System.turn()`, a blank line, and a short fixed instruction to continue the conversation. The message MUST be byte-stable across turns. The list therefore always ends on a user turn; several chat templates end the assistant turn immediately when the final message is system-role.
