...
Replace synchronous delegation (a parent agent blocking on a child agent) with a job model patterned after Sidekiq. Work to be done lives in a single task docmem as a tree of task nodes. A harness owns that docmem for its lifetime, repeatedly picks the next task in depth-first order, runs an agent on it, and hands control back to the tree. Agents shape the tree as they go: they decompose a task by adding child tasks, defer a task by moving it, and finish a task by folding it under a summary node. There is no call stack and no master agent; all state is in the docmem. See Supersedes for what this replaces.
...
- **Summary node**: a node with `context_type = summary` inserted above a finished task using the docmem summarize operation (SPEC_DOCMEM, Summary Operations; the operation accepts non-leaf nodes). The original task subtree is preserved beneath it. The harness never descends into a summary node, and context serialization omits its children, so the tree compresses as work completes.
- **State block**: a `{key=value, ...}` block at the start of a task node's text, read and written by both the harness and agents. Specified under Task Docmem Structure.
...
- Task: `context_type = task`, `context_name` the lens name (MAY be empty; the harness passes it through, lens docmems are specified elsewhere), `context_value` unused (reserved).
...
| `retries`  | agent, user     | per-task override of the retry limit (default 3)                  |
...
followed by a short fixed instruction block explaining around advice, the state block, and the `suspend` and `finish` commands. The task node itself, with its state block and instruction, is not repeated because it is already visible in the task docmem context; the message carries the two ids so the agent can address its own node and its own chat with the docmem tools.
...
- `finish(summary)` ends the run and folds the task. `summary` is required. Its content is at the agent's discretion; it SHOULD say what was done in enough detail that later tasks and the parent need not peek beneath the summary.
...
Every run ends in exactly one of these ways. The harness writes the state block accordingly and, where noted, folds the task under a summary node.
...
- The retry limit defaults to 3 and MAY be overridden per task with the `retries` key.
...
- A task returned to `queued` runs before its own children (preorder), even if the interrupted or failed run created some. Its `chat` key is kept, so the agent continues the same conversation and sees what it already did.
...
- The harness has two states: running and stopped. Run starts the selection loop; Stop halts it. Both live on the Tasks panel.
...
- Run MUST reset every task with `status=running` to `queued` before selecting. A `running` status at that point can only be stale: left by a TOML snapshot taken mid-run, a reload, or a crash. Without this reset such a task would never be eligible again.
