...
## 2026-09-14: Reserved node reference `last`

**Delta.** The command router resolves the literal `last` in any node-id argument to the id of the node most recently placed or content-updated by that router in the current response. The placing commands are create node, copy, move, and add summary; update content also sets it; every other command leaves the reference alone. Command routers are now objects with `run(args, docmem)` and `beginResponse()`; `AgentLoop` calls `beginResponse()` before executing a response's calls, which clears the reference. Using it with nothing to point at fails with `no last node in this response`. Docmem refuses to insert any node with id `last`, root or interior, on every creation path, so the reference can never be shadowed. `DocmemCommands` results that set the reference carry a `lastId` field so the router learns the id without parsing result text. Results print the resolved id. The prompt, SPEC_DOCMEM, and a router test page document it.

**Rationale.** Two `after X` inserts on the same anchor land reversed, and a DeepSeek v4 Pro worker spent turns on structure dumps repairing that. `last` lets a model chain in-order inserts in one response without waiting for ids. Interior ids stay random and anonymous; the reference is transient stack space, scoped to one response so a stale handle from an earlier turn cannot misplace a node, and by the next turn the model has the real ids in its results.

## 2026-09-14: Node ids are global; commands carry no docmem binding

**Delta.** `DocmemCommands` no longer takes a `Docmem`. It runs over `DocmemStore`, a new root-free base class holding every node operation; `Docmem` extends it with only the root handle (`init`, `createRoot`, `getRoot`). Each router owns one `DocmemCommands` for its lifetime. Six command methods no caller reached (`appendChild`, `insertBefore`, `insertAfter`, `moveAppendChild`, `moveBefore`, `moveAfter`) are removed; `createNode` and `moveNode` cover them. Command routers are `{ run(args), beginResponse() }`; `AgentLoop` and `TaskHarness` no longer pass the chat docmem through. The router's carve-out for commands that worked without a bound docmem is gone with the binding. SPEC_DOCMEM states that ids are global and that confinement to one docmem comes only from explicit root arguments or same-root checks. The router test page pins that one router edits a node in any docmem by id and that move still refuses to cross roots.

**Rationale.** The binding was vestigial. Every node lookup already went by id alone against the single shared table, so a command on a docmem-bound instance could edit or delete a node in any other docmem, while the class shape suggested otherwise. The commands that do confine (`docmem_focus` with its root argument, `docmem_move_node` with its same-root check) do so explicitly. Making the class honest removes a parameter from every router call, and leaves the read-set as the mechanism that decides which ids a worker can see, consistent with keeping context under harness and user control.
