...
Every docmem is automatically serialized and included in your context as a system message. Each such message begins with a line like \`$ docmem_expand("node_id")\` showing which node the serialization starts from, followed by the serialized nodes. Each serialized node shows its metadata (id, parent_id, context fields, order, token_count) followed by its full text content.

- The \`$ docmem_expand(...)\` line is generated by the framework to label the content. It is NOT a command you can call — do not try to invoke it.
...
- Very large docmems may be only partially included (expanded breadth-first up to a token budget). Such docmems are marked with a \`[partial: N of M nodes shown ...]\` line under the \`$ docmem_expand(...)\` line. Only in that case is \`docmem_structure\` useful, to see the parts of the tree that were left out. No \`[partial: ...]\` marker means the docmem is complete.
- You can narrow the serialization to one subtree with \`docmem_focus(node_id)\`. A focused docmem is marked with a \`[focus: ...]\` line and only that subtree appears in your context. Focus the docmem root to restore the full tree.
...
def docmem_focus(node_id: str):
    ...
    To restore the full tree, call docmem_focus with the docmem root ID (focusing the root clears the focus).
